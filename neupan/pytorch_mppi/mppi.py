...
class MPPI():
    ...
    def _compute_rollout_costs(self, perturbed_actions):
        ...
        # Actions is M * K x T x nu
        # States is M * K x T x nx
        actions = torch.stack(actions, dim=-2)
        states = torch.stack(states, dim=-2)   
                
        t = torch.arange(T, dtype=self.dtype, device=self.d)
                
        c = self._running_cost(states, actions, t) # (M, K, T)
        cost_samples = c.sum(dim=-1) # (M, K)
                
        if self.M > 1:
            var_t = c.var(dim=0, unbiased=True) # (K, T)
            discounts = (self.rollout_var_discount ** torch.arange(T, device=self.d)).view(1, T) # (1, T)
            cost_var = (var_t * discounts).sum(dim=1) # (K,)
